...
import DCRSApp.corbaInterfacePOA;
...
//	ArrayList<String> arrl = new ArrayList<String>(3);
	...
	public boolean removeCourse(String courseID1, String semester1) {
		String courseID = courseID1.trim().toUpperCase();
		String semester = semester1.trim().toUpperCase();
		String Result = "fail";
		String AdvDept =courseID.substring(0,4);
		if(AdvDept.equals("COMP")) {
			fetchValues = CompServer.hashComp.get(semester);
	...
	public synchronized static boolean Student(String studentID, String courseID, String semester){
		HashMap<String, HashSet<String>> studinner = new HashMap<String, HashSet<String>>();
		String Result = "fail";
		int counter=0;
...
/*	@Override
	...
	public String getClassSchedule(String studentID) {
		...
		if(!hashStudent.containsKey(studentID)) {
			...
			Result=studentID + " " + "is not enrolled in any of the courses.";
		}else {
			studinner = hashStudent.get(studentID);
			/*if(studinner.containsKey("FALL")) {
			fetchCourseF = studinner.get("FALL");}
...
*/		}
	...
	public synchronized boolean enrolCourse(String studentID1, String courseID1, String semester1) {
		String studentID = studentID1.trim().toUpperCase();
		String courseID = courseID1.trim().toUpperCase();
		String semester = semester1.trim().toUpperCase();
		String Result = "fail";
		String s = null;
...
//		HashMap<String, HashMap<String, HashSet<String>>> hashStudent= new HashMap<String, HashMap<String, HashSet<String>>>();
	...
	public synchronized boolean swapCourse(String studentID1, String oldcourseID1, String newcourseID1) {
		String studentID = studentID1.trim().toUpperCase();
		String oldcourseID = oldcourseID1.trim().toUpperCase().trim();
		String newcourseID = newcourseID1.trim().toUpperCase().trim();
		System.out.println("old course"+oldcourseID);
		System.out.println("old course"+newcourseID);
		String Result = "fail";
		String Result1 = "fail";
		String s = null;
		String StuDept = null;
		...
		if(!hashStudent.containsKey(studentID)) {
			...
			if(studinner.containsKey("FALL")) {
				...
				fetchCourseS = studinner.get("SUMMER");}
				System.out.println(fetchCourseS);
				if(fetchCourseF.contains(oldcourseID.trim())) {
					semester = "FALL";
					fetchCourseF.remove(oldcourseID.trim());
					studinner.put("FALL",fetchCourseF);
					hashStudent.put(studentID,studinner);
					flag =true;}
				if(fetchCourseW.contains(oldcourseID.trim())) {
					semester = "WINTER";
					fetchCourseW.remove(oldcourseID);
					studinner.put("WINTER",fetchCourseW);
					hashStudent.put(studentID,studinner);
					flag =true;}
				if(fetchCourseS.contains(oldcourseID.trim())) {
					semester = "SUMMER";
					fetchCourseS.remove(oldcourseID.trim());
					studinner.put("SUMMER",fetchCourseS);
					hashStudent.put(studentID,studinner);
					flag =true;}
				...
				if (flag){
					Result1 = "pass";
				System.out.println(studentID + " " + "successfully removed course" + " " + oldcourseID.trim());
				StuDept = studentID.substring(0,4).toUpperCase();
				if(StuDept.equals("COMP")) {
					String L =CompServer.DropCourse(oldcourseID.trim(),semester);
					System.out.println(L);
				}else if(StuDept.equals("INSE")) {
					String M =InseServer.DropCourse(oldcourseID.trim(),semester);
					System.out.println(M);
				}else if(StuDept.equals("SOEN")) {
					String N =SoenServer.DropCourse(oldcourseID.trim(),semester);
	...
	public synchronized String swapCourse(String studentID, String oldcourseID, String newcourseID) {
		...
		if(!hashStudent.containsKey(studentID)) {
			...
			if(studinner.containsKey("FALL")) {
				...
				System.out.println("swap flag is:"+ flag);
				if (flag){
				//	s = enrollCourse(studentID, newcourseID, semester);
					s = dropCourse(studentID, oldcourseID);
					System.out.println("SWAP-DROP"+hashStudent);
					if(s.equals("pass")) {
				//	String m =	dropCourse(studentID, oldcourseID);
					String m = enrollCourse(studentID, newcourseID, semester);
					System.out.println("SWAP-ENROLL1"+hashStudent);
					if(m.equals("pass")) {
						System.out.println("Swapping successful!");
						Result = "Swapping successful!";
						System.out.println("SWAP-ENROLL2"+hashStudent);
					}else {
					//	String o =	dropCourse(studentID, newcourseID);
						String o = enrollCourse(studentID, oldcourseID, semester);
					//	System.out.println("new course enrolled and then dropped-Swapping failed!");
						System.out.println("old course dropped and then enrolled again-Swapping failed!");
					}
					}else {
						System.out.println("Student could not be enrolled in new course-Swapping failed!");
					...
					System.out.println("Student is not enrolled course-"+" "+oldcourseID);
				}
		}
		return Result;
	}*/
